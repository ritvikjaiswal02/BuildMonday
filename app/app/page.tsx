...
export default function AppPage() {
  ...
  const headerRight = hasResults ? (
    <>
      <button
        ...
        className="cursor-pointer rounded-md border border-indigo-500/40 bg-indigo-500/15 px-3 py-1.5 text-xs font-medium text-indigo-200 transition hover:bg-indigo-500/25"
      ...
      <button
        onClick={copyReport}
        className="cursor-pointer rounded-md border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/80 transition hover:bg-white/10"
  ...
  return (
    <>
      {/* Sparkles background — fixed, behind everything */}
      <div className="pointer-events-none fixed inset-0 z-0 bg-[#020617]">
        ...
        />
        {/* Radial mask so sparkles fade toward the edges and behind content */}
        <div
          className="absolute inset-0"
          style={{
            background:
              "radial-gradient(ellipse 70% 50% at 50% 30%, transparent 0%, rgba(2,6,23,0.55) 60%, rgba(2,6,23,0.95) 100%)",
          }}
        />
        {/* Ambient color glow to match the brand */}
        <div
          className="absolute inset-x-0 top-0 h-[520px] opacity-60"
          style={{
            background:
              "radial-gradient(ellipse 60% 60% at 50% 0%, rgba(99,102,241,0.22), transparent 70%), radial-gradient(ellipse 40% 50% at 80% 20%, rgba(245,158,11,0.10), transparent 70%)",
            filter: "blur(20px)",
          }}
        />
      </div>
      ...
      <main className="mx-auto min-h-[calc(100vh-60px)] max-w-3xl px-4 py-10 sm:py-14">
        {!hasResults ? (
          <div className="space-y-3">
            <motion.div
              ...
              className="mb-3"
            >
              <LivePill label="New analysis" />
              <h1
                className="mt-4 font-display text-[2.4rem] font-medium leading-[1.05] tracking-[-0.025em] sm:text-[3.2rem]"
                style={{
                  backgroundImage:
                    "linear-gradient(to bottom, #ffffff 0%, #ffffff 60%, rgba(255,255,255,0.55) 100%)",
              ...
              >
                ...
                <span className="relative inline-block">
                  <span
                    ...
                    style={{
                      background:
                        "radial-gradient(ellipse 60% 80% at 50% 50%, rgba(252,211,77,0.55), transparent 70%)",
                    }}
                  />
                  <span
              ...
              <p className="mt-3 text-[15px] text-white/65">
                Paste raw complaints, or pull live reviews from any URL.
              </p>
            </motion.div>
            <motion.div
          ...
          <motion.div
            ...
            className="mb-6"
          >
            <LivePill label="Results" tone="emerald" />
            <h1
              className="mt-3 font-display text-3xl font-medium leading-[1.1] tracking-[-0.02em] sm:text-[2.4rem]"
              style={{
                backgroundImage:
                  "linear-gradient(to bottom, #ffffff 0%, #ffffff 65%, rgba(255,255,255,0.6) 100%)",
                WebkitBackgroundClip: "text",
                WebkitTextFillColor: "transparent",
                backgroundClip: "text",
              }}
        ...
        {sharedView && hasResults && (
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-4 py-3 text-sm text-indigo-100">
            <span>
              ...
              <span className="font-semibold">{issues.length}</span> issues · PRDs
              not included
            </span>
            <button
              onClick={startOver}
            ...
            {diff && diff.resolved.length > 0 && (
              <div className="mt-6 rounded-xl border border-white/10 bg-[#0e0e10] px-4 py-4">
                ...
                <p className="mt-1 text-xs text-white/40">
                  Clusters from the previous analysis that did not reappear this
                  time.
                </p>
  ...
  const palette =
    tone === "emerald"
      ? {
          ...
          dot: "bg-emerald-300",
          ping: "bg-emerald-300/70",
          text: "text-emerald-100/90",
          shadow: "shadow-[0_0_24px_-6px_rgba(110,231,183,0.45)]",
        }
      : {
          ...
          shadow: "shadow-[0_0_24px_-6px_rgba(252,211,77,0.45)]",
        };
  return (
    <motion.div
      ...
      className={`inline-flex items-center gap-2 rounded-full border ${palette.border} ${palette.bg} ${palette.shadow} px-3 py-1 backdrop-blur-sm`}
    >
      <span className="relative flex h-1.5 w-1.5">
        <span
          className={`absolute inset-0 animate-ping rounded-full ${palette.ping}`}
        />
        <span
      ...
      <span
        className={`text-[10px] font-semibold uppercase tracking-[0.24em] ${palette.text}`}
      >
        {label}
      </span>
    </motion.div>
